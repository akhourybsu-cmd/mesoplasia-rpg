#!/usr/bin/env python3
"""Build the external review package for the inactive Commons terrain gate."""
...
PREP_TOOL = ROOT / "tools/art/prepare_caden_commons_terrain_gate_v1_2.py"
RENDER_TOOL = ROOT / "tools/art/render_caden_commons_terrain_gate_v1_2.gd"
COMMONS_TEST = ROOT / "tests/caden_commons_runtime_test.gd"

PAIRS = (
    ("commons_terrain_full_current_v1_2_1024x704.png", "commons_terrain_full_candidate_v1_2_1024x704.png", "Full Commons", 455),
    ("commons_terrain_west_current_v1_2_640x360.png", "commons_terrain_west_candidate_v1_2_640x360.png", "Town Square arrival", 405),
    ("commons_terrain_north_current_v1_2_640x360.png", "commons_terrain_north_candidate_v1_2_640x360.png", "Residential arrival", 405),
    ("commons_terrain_junction_current_v1_2_640x360.png", "commons_terrain_junction_candidate_v1_2_640x360.png", "Route junction and Player", 405),
    ("commons_terrain_quiet_green_current_v1_2_640x360.png", "commons_terrain_quiet_green_candidate_v1_2_640x360.png", "Quiet Green and rest pocket", 405),
    ("commons_terrain_south_current_v1_2_640x360.png", "commons_terrain_south_candidate_v1_2_640x360.png", "Southern planted boundary", 405),
)
...
def verify_prep(prep_root: Path) -> dict:
    manifest_path = prep_root / "metadata/commons_terrain_preparation_manifest_v1_2.json"
    manifest = load_json(manifest_path)
    if manifest.get("gate_state") != "inactive_commons_terrain_comparison_pending_visual_approval":
        raise RuntimeError("Unexpected Commons preparation gate state.")
    if manifest.get("generator_sha256") != sha256(PREP_TOOL):
        raise RuntimeError("Commons preparation tool changed after candidate generation.")
    ...
    if not candidate_path.is_file() or sha256(candidate_path) != candidate.get("runtime_sha256", ""):
        raise RuntimeError("Commons terrain candidate identity mismatch.")
    if candidate.get("runtime_dimensions") != [1024, 704] or candidate.get("route_footprint_cells") != 108:
        raise RuntimeError("Commons candidate dimensions or route footprint changed.")
    for relative, digest in manifest.get("boards", {}).items():
        path = prep_root / relative
        if not path.is_file() or sha256(path) != digest:
            raise RuntimeError(f"Commons preparation board mismatch: {relative}")
    return manifest


def verify_render(render_root: Path, prep_manifest: dict) -> dict:
    manifest_path = render_root / "commons_terrain_render_manifest_v1_2.json"
    manifest = load_json(manifest_path)
    if manifest.get("gate_state") != "inactive_commons_terrain_comparison_pending_visual_approval" or manifest.get("active_reference_changed") is not False:
        raise RuntimeError("Unexpected Commons render gate state.")
    if manifest.get("render_tool_sha256") != sha256(RENDER_TOOL):
        raise RuntimeError("Commons render tool changed after capture.")
    if manifest.get("candidate_terrain_sha256") != prep_manifest["candidate"]["runtime_sha256"]:
        raise RuntimeError("Rendered Commons candidate identity mismatch.")
    captures = manifest.get("captures", [])
    if len(captures) != 13:
        raise RuntimeError("Expected thirteen Commons terrain captures.")
    for record in captures:
        path = render_root / "raw_captures" / record["filename"]
        if not path.is_file() or sha256(path) != record["sha256"]:
            raise RuntimeError(f"Commons capture mismatch: {path.name}")
    junction = Image.open(render_root / "raw_captures/commons_terrain_junction_candidate_v1_2_640x360.png").convert("RGBA")
    display = Image.open(render_root / "raw_captures/commons_terrain_junction_candidate_display_v1_2_1280x720.png").convert("RGBA")
    if junction.resize((1280, 720), Image.Resampling.NEAREST).tobytes() != display.tobytes():
        raise RuntimeError("Commons 1280x720 proof is not an exact nearest-neighbor 2x image.")
    return manifest
...
def verify_active_reference(prep_manifest: dict, render_manifest: dict) -> None:
    ...
    if sha256(CURRENT_TERRAIN) != current_manifest.get("output_sha256"):
        raise RuntimeError("Active Commons terrain bytes changed.")
    ...
    if scene_text.count(active_reference) != 1 or "commons_terrain_runtime_v1_2" in scene_text:
        raise RuntimeError("Inactive Commons candidate entered the active scene.")
    scene_hash = sha256(SCENE_PATH)
    if prep_manifest.get("scene_sha256") != scene_hash or render_manifest.get("scene_sha256") != scene_hash:
        raise RuntimeError("Commons scene changed during comparison preparation.")
...
def build_comparison_board(render_root: Path, output_path: Path) -> None:
    ...
    height = title_height + sum(label_height + pair[3] + 34 for pair in PAIRS) + 82
    board = Image.new("RGB", (width, height), (239, 237, 229))
    draw = ImageDraw.Draw(board)
    draw.text((margin, 26), "CADEN COMMONS TERRAIN GATE v1.2", font=font(32, True), fill=(40, 46, 41))
    draw.text((margin, 70), "Inactive comparison. Quiet Green, exact routes, scenery, population, collision, and transitions remain authoritative.", font=font(18), fill=(75, 79, 73))
    y = title_height
    capture_root = render_root / "raw_captures"
    for current_name, candidate_name, title, image_height in PAIRS:
        ...
        y += image_height + 34
    draw.text((margin, height - 58), "Approval gate: accept a limited Commons terrain pilot, request correction, or retain Commons Runtime v1.", font=font(18, True), fill=(91, 66, 39))
...
def write_docs(output_root: Path, candidate_hash: str) -> None:
    ...
    (docs / "README.md").write_text(
        ...
        "This package compares the current Commons Runtime v1 terrain with an inactive source-derived v1.2 candidate. "
        "The candidate uses the approved harmonized material family while preserving the exact 108-cell Town Square and Residential route footprint and 596-cell grass field.\n\n"
        "The Quiet Green remains open and dominant. All selected scenery, layered perimeter planting, three visible residents, collision, depth sorting, entries, and exits remain unchanged. "
        "No building replacement is proposed for the Commons.\n\n"
        f"Candidate SHA-256: `{candidate_hash}`.\n",
        encoding="utf-8", newline="\n",
    )
    (docs / "APPROVAL_CHECKLIST.md").write_text(
        "# Visual Approval Checklist\n\n"
        "- [ ] The Commons remains rural and quieter than Marketplace and Town Square.\n"
        "- [ ] The Quiet Green still reads as broad, open, and walkable.\n"
        "- [ ] Warm stone is limited to the exact existing route footprint.\n"
        "- [ ] Town Square and Residential arrivals remain unmistakable and open.\n"
        "- [ ] The Player and residents remain legible at `640 x 360`.\n"
        "- [ ] Grass/stone boundaries have no gaps, halos, hard rectangular seams, or clipped corners.\n"
        "- [ ] The material family improves cross-zone cohesion without making the Commons urban.\n\n"
        "Decision: ACCEPT LIMITED COMMONS TERRAIN PILOT / TARGETED CORRECTION / RETAIN RUNTIME v1\n",
        encoding="utf-8", newline="\n",
    )
...
def build_package(prep_root: Path, render_root: Path, output_root: Path) -> Path:
    ...
    shutil.copy2(prep_root / "metadata/commons_terrain_preparation_manifest_v1_2.json", metadata_root)
    shutil.copy2(render_root / "commons_terrain_render_manifest_v1_2.json", metadata_root)
    tooling_root = output_root / "tooling"
    tooling_root.mkdir(parents=True, exist_ok=True)
    for path in (PREP_TOOL, RENDER_TOOL, SCRIPT_PATH, COMMONS_TEST):
        shutil.copy2(path, tooling_root / path.name)

    comparison_board = output_root / "boards/commons_terrain_comparison_v1_2.png"
    ...
    package_manifest = {
        "schema": "caden-commons-terrain-review-package-v1.2",
        "gate_state": "inactive_commons_terrain_comparison_pending_visual_approval",
        "scope": "Commons terrain comparison only; no active scene or resource change.",
        "candidate_runtime_sha256": candidate_hash,
        "current_runtime_sha256": sha256(CURRENT_TERRAIN),
        "scene_sha256": sha256(SCENE_PATH),
        "preparation_manifest_sha256": sha256(prep_root / "metadata/commons_terrain_preparation_manifest_v1_2.json"),
        "render_manifest_sha256": sha256(render_root / "commons_terrain_render_manifest_v1_2.json"),
        ...
        "tool_hashes": {f"tooling/{path.name}": sha256(path) for path in (PREP_TOOL, RENDER_TOOL, SCRIPT_PATH, COMMONS_TEST)},
        "provenance_and_licensing": prep_manifest["provenance_and_licensing"],
        "visual_decision_required": "accept limited Commons terrain pilot, request targeted correction, or retain Runtime v1",
    }
