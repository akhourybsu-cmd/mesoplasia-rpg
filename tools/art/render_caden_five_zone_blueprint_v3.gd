...
const TOOL_PATH := "res://tools/art/render_caden_five_zone_blueprint_v3.gd"
const OVERLAY_PATH := "res://scenes/world/caden/comparisons/TownSquareBlueprintV3Overlay.tscn"
...
func _render() -> void:
	...
	specs.append_array(_zone_specs(1, "wayfarers_approach", "Wayfarer's Approach", WAYFARER_SCENE,
		"res://scenes/world/caden/WayfarersApproach.tscn", Vector2i(1024, 640), Vector2(512, 320),
		Vector2(512, 320), Vector2(512, 320), Vector2(704, 460), Vector2(720, 500)))
	specs.append_array(_zone_specs(2, "marketplace", "Marketplace", MARKETPLACE_SCENE,
		"res://scenes/world/caden/Marketplace.tscn", Vector2i(896, 640), Vector2(448, 320),
		Vector2(448, 320), Vector2(448, 320), Vector2(448, 460), Vector2(448, 416)))
	specs.append_array(_zone_specs(3, "town_square", "Town Square", TOWN_SQUARE_SCENE,
		"res://scenes/world/caden/TownSquare.tscn", Vector2i(960, 704), Vector2(480, 352),
		Vector2(480, 352), Vector2(480, 352), Vector2(640, 288), Vector2(640, 320)))
	specs.append_array(_zone_specs(4, "residential", "Residential", RESIDENTIAL_SCENE,
		"res://scenes/world/caden/Residential.tscn", Vector2i(1152, 768), Vector2(576, 384),
		Vector2(576, 384), Vector2(576, 384), Vector2(320, 608), Vector2(320, 640)))
	specs.append_array(_zone_specs(5, "commons", "Commons", COMMONS_SCENE,
		"res://scenes/world/caden/Commons.tscn", Vector2i(1024, 704), Vector2(512, 352),
		Vector2(512, 352), Vector2(512, 352), Vector2(768, 416), Vector2(768, 448)))
	specs.append(_spec(16, "town_square_approved_civic_garden_full_960x704", "Town Square approved civic garden",
		TOWN_SQUARE_SCENE, "res://scenes/world/caden/TownSquare.tscn", Vector2(480, 352), NO_PLAYER,
		Vector2i(960, 704), true))
	specs.append(_spec(17, "town_square_approved_civic_garden_feature_640x360", "Town Square approved civic garden",
		TOWN_SQUARE_SCENE, "res://scenes/world/caden/TownSquare.tscn", Vector2(700, 288), Vector2(640, 320),
		GAMEPLAY_SIZE, true))
	...
	var manifest := {
		"schema": "caden-five-zone-blueprint-v3-review",
		"gate_state": "active_four_zone_pass_with_user_approved_town_square_civic_garden",
		"renderer": "Compatibility",
		"render_tool": TOOL_PATH.trim_prefix("res://"),
		"render_tool_sha256": FileAccess.get_sha256(TOOL_PATH),
		"town_square_overlay": OVERLAY_PATH.trim_prefix("res://"),
		"town_square_overlay_sha256": FileAccess.get_sha256(OVERLAY_PATH),
		"capture_count": captures.size(),
		"captures": captures,
	}
	var manifest_path := _output_root.path_join("caden_five_zone_blueprint_v3_review.json")
	var file := FileAccess.open(manifest_path, FileAccess.WRITE)
	if file == null:
		_fail("Unable to create review manifest.")
		return
...
func _zone_specs(zone_index: int, slug: String, zone_name: String, scene: PackedScene, scene_path: String,
		full_size: Vector2i, full_camera: Vector2, primary_camera: Vector2, primary_player: Vector2,
		feature_camera: Vector2, feature_player: Vector2) -> Array[Dictionary]:
	var start_id := (zone_index - 1) * 3 + 1
	return [
		_spec(start_id, "%s_full_zone_%dx%d" % [slug, full_size.x, full_size.y], zone_name, scene, scene_path,
			full_camera, NO_PLAYER, full_size, false),
		_spec(start_id + 1, "%s_primary_640x360" % slug, zone_name, scene, scene_path,
			primary_camera, primary_player, GAMEPLAY_SIZE, false),
		_spec(start_id + 2, "%s_feature_640x360" % slug, zone_name, scene, scene_path,
			feature_camera, feature_player, GAMEPLAY_SIZE, false),
	]


func _spec(evidence_id: int, name: String, zone_name: String, scene: PackedScene, scene_path: String,
		camera_center: Vector2, player_position: Vector2, capture_size: Vector2i, approval_focus: bool) -> Dictionary:
	return {
...
func _manifest_entry(spec: Dictionary, output_path: String) -> Dictionary:
	...
	return {
		...
		"approved_civic_garden_focus": spec["approval_focus"],
