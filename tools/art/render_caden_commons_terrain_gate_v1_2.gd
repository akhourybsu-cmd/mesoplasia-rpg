extends "res://tools/art/render_caden_commons_runtime_v1.gd"

const PREP_TOOL_PATH := "res://tools/art/prepare_caden_commons_terrain_gate_v1_2.py"
const ACTIVE_TERRAIN_PATH := "res://assets/environments/caden/commons/terrain/commons_terrain_runtime_v1.png"
const ACTIVE_TERRAIN_MANIFEST_PATH := "res://assets/environments/caden/commons/terrain/commons_terrain_runtime_v1.json"
const TERRAIN_GATE_RENDER_TOOL_PATH := "res://tools/art/render_caden_commons_terrain_gate_v1_2.gd"
...
func _render() -> void:
	...
	var prep_manifest_path := prep_root.path_join("metadata/commons_terrain_preparation_manifest_v1_2.json")
	var prep_manifest := _load_external_json(prep_manifest_path)
	if prep_manifest.get("gate_state", "") != "inactive_commons_terrain_comparison_pending_visual_approval":
		_fail("Unexpected Commons terrain preparation gate state.")
		return
	if prep_manifest.get("generator_sha256", "") != FileAccess.get_sha256(PREP_TOOL_PATH):
		_fail("Commons terrain preparation tool changed after candidate generation.")
		return
	...
	if FileAccess.get_sha256(candidate_path) != candidate_record.get("runtime_sha256", ""):
		_fail("Commons terrain candidate hash mismatch.")
		return
	...
	if candidate_image == null or candidate_image.get_size() != FULL_ZONE_SIZE:
		_fail("Commons terrain candidate dimensions are invalid.")
		return
	...
	if FileAccess.get_sha256(ACTIVE_TERRAIN_PATH) != active_manifest.get("output_sha256", ""):
		_fail("Active Commons terrain identity mismatch.")
		return
	...
	if active_scene_texture == null or active_scene_texture.resource_path != ACTIVE_TERRAIN_PATH:
		active_scene.free()
		_fail("Serialized Commons scene does not retain Terrain Runtime v1.")
		return
	active_scene.free()
	...
	var specs: Array[Dictionary] = [
		_comparison_spec("commons_terrain_full_current_v1_2_1024x704", false, Vector2(512, 352), NO_PLAYER, FULL_ZONE_SIZE, FULL_ZONE_SIZE, "Full approved Commons Runtime v1"),
		_comparison_spec("commons_terrain_full_candidate_v1_2_1024x704", true, Vector2(512, 352), NO_PLAYER, FULL_ZONE_SIZE, FULL_ZONE_SIZE, "Full inactive terrain candidate"),
		_comparison_spec("commons_terrain_west_current_v1_2_640x360", false, Vector2(320, 352), Vector2(128, 352), GAMEPLAY_SIZE, GAMEPLAY_SIZE, "Town Square arrival current"),
		_comparison_spec("commons_terrain_west_candidate_v1_2_640x360", true, Vector2(320, 352), Vector2(128, 352), GAMEPLAY_SIZE, GAMEPLAY_SIZE, "Town Square arrival candidate"),
		_comparison_spec("commons_terrain_north_current_v1_2_640x360", false, Vector2(512, 256), Vector2(512, 128), GAMEPLAY_SIZE, GAMEPLAY_SIZE, "Residential arrival current"),
		_comparison_spec("commons_terrain_north_candidate_v1_2_640x360", true, Vector2(512, 256), Vector2(512, 128), GAMEPLAY_SIZE, GAMEPLAY_SIZE, "Residential arrival candidate"),
		_comparison_spec("commons_terrain_junction_current_v1_2_640x360", false, Vector2(512, 352), Vector2(512, 352), GAMEPLAY_SIZE, GAMEPLAY_SIZE, "Primary route junction and player current"),
		_comparison_spec("commons_terrain_junction_candidate_v1_2_640x360", true, Vector2(512, 352), Vector2(512, 352), GAMEPLAY_SIZE, GAMEPLAY_SIZE, "Primary route junction and player candidate"),
		_comparison_spec("commons_terrain_quiet_green_current_v1_2_640x360", false, Vector2(704, 352), NO_PLAYER, GAMEPLAY_SIZE, GAMEPLAY_SIZE, "Quiet Green and rest pocket current"),
		_comparison_spec("commons_terrain_quiet_green_candidate_v1_2_640x360", true, Vector2(704, 352), NO_PLAYER, GAMEPLAY_SIZE, GAMEPLAY_SIZE, "Quiet Green and rest pocket candidate"),
		_comparison_spec("commons_terrain_south_current_v1_2_640x360", false, Vector2(512, 524), NO_PLAYER, GAMEPLAY_SIZE, GAMEPLAY_SIZE, "Southern planted boundary current"),
		_comparison_spec("commons_terrain_south_candidate_v1_2_640x360", true, Vector2(512, 524), NO_PLAYER, GAMEPLAY_SIZE, GAMEPLAY_SIZE, "Southern planted boundary candidate"),
		_comparison_spec("commons_terrain_junction_candidate_display_v1_2_1280x720", true, Vector2(512, 352), Vector2(512, 352), GAMEPLAY_SIZE, DISPLAY_SIZE, "Exact nearest-neighbor 2x candidate junction view"),
	]
	...
	var manifest := {
		"schema": "caden-commons-terrain-gate-v1.2-render",
		"gate_state": "inactive_commons_terrain_comparison_pending_visual_approval",
		"scope": "Transient Commons terrain comparison only; active scene and resources remain unchanged.",
	...
	var manifest_path := _output_root.path_join("commons_terrain_render_manifest_v1_2.json")
	var manifest_file := FileAccess.open(manifest_path, FileAccess.WRITE)
	if manifest_file == null:
		_fail("Unable to create Commons terrain render manifest.")
		return
...
func _comparison_spec(name: String, candidate: bool, camera_center: Vector2, player_position: Vector2, capture_size: Vector2i, output_size: Vector2i, description: String) -> Dictionary:
	return {
		"name": name, "candidate": candidate, "camera_center": camera_center,
		"player_position": player_position, "capture_size": capture_size,
		"output_size": output_size, "description": description,
	}
