...
const RENDER_TOOL_PATH := "res://tools/art/render_caden_commons_baseline_v1.gd"
...
func _render() -> void:
	...
	var specs: Array[Dictionary] = [
		_spec("commons_full_zone_before_v1_1024x704", Vector2(512, 352), NO_PLAYER, FULL_ZONE_SIZE, FULL_ZONE_SIZE, "Untouched Commons full-zone contract"),
		_spec("commons_west_arrival_before_v1_640x360", Vector2(320, 352), Vector2(128, 352), GAMEPLAY_SIZE, GAMEPLAY_SIZE, "Town Square arrival and west-east route"),
		_spec("commons_north_arrival_before_v1_640x360", Vector2(512, 256), Vector2(512, 128), GAMEPLAY_SIZE, GAMEPLAY_SIZE, "Residential arrival and north-south route"),
		_spec("commons_route_junction_before_v1_640x360", Vector2(512, 352), Vector2(512, 352), GAMEPLAY_SIZE, GAMEPLAY_SIZE, "Primary route junction and quiet-green approach"),
		_spec("commons_quiet_green_before_v1_640x360", Vector2(704, 352), NO_PLAYER, GAMEPLAY_SIZE, GAMEPLAY_SIZE, "Quiet Green reserved open space"),
		_spec("commons_south_green_before_v1_640x360", Vector2(512, 524), NO_PLAYER, GAMEPLAY_SIZE, GAMEPLAY_SIZE, "Southern negative space and boundary"),
		_spec("commons_route_display_before_v1_1280x720", Vector2(512, 352), Vector2(512, 352), GAMEPLAY_SIZE, DISPLAY_SIZE, "Exact nearest-neighbor 2x route-junction view"),
	]
	...
	var manifest := {
		"schema": "caden-commons-baseline-screenshot-manifest-v1",
		"scope": "Untouched Commons reconstruction baseline.",
		...
		"determinism": {"player_facing": "down", "player_animation": "idle_down", "player_animation_frame": 0, "npc_animation_frame": 0, "ui_state": "interaction prompt and dialogue UI hidden", "camera_smoothing": "disabled capture camera"},
		"reproduction": {
			"godot_arguments": "--path <repository> --rendering-method gl_compatibility --script res://tools/art/render_caden_commons_baseline_v1.gd -- --output-root <absolute-directory-outside-res>",
			"notes": "Every view instantiates the untouched live Commons scene. The 1280x720 frame is nearest-neighbor resized from its native 640x360 capture state.",
		},
		"captures": entries,
	}
	var manifest_path := _output_root.path_join("commons_baseline_screenshot_manifest_v1.json")
...
func _resolve_output_root() -> String:
	...
	return OS.get_environment("CADEN_COMMONS_BASELINE_OUTPUT_ROOT")
