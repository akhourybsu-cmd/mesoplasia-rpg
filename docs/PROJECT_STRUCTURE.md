...
│   └── expeditions/
├── config/
│   └── dedicated_server.example.json
├── docs/
├── scenes/
│   ├── development/
│   │   ├── CadenResourceSandbox.tscn
│   │   ├── LocalMultiAvatarSandbox.tscn
│   │   ├── OfflineCombatSandbox.tscn
│   │   └── PersistenceSandbox.tscn
│   ├── network/
│   │   ├── NetworkCombatSandbox.tscn
...
│   │   ├── NetworkedCombatPresenter.tscn
│   │   ├── NetworkedExpeditionPresenter.tscn
│   │   └── NetworkRuntime.tscn
│   ├── server/
│   │   └── DedicatedServer.tscn
│   ├── world/
│   │   ├── caden/
│   │       ├── Caden.tscn
│   │       ├── Commons.tscn
...
- `data/` contains structured game-content data. Phase F adds one registry-backed authored development expedition under `data/expeditions/`; it is content data rather than narrative canon or procedural-generation logic. Phase G adds minimal non-canon ability, status, and combatant fixtures under `data/combat/` for deterministic domain verification. Phase K adds one development resource and one development fortification-probe definition under `data/caden/`; neither establishes final economy, siege, or canon content.
